...
def extrair_contrato(texto):
    ...
    padroes = [
    r'\bCONTRATO(?:\s+ADMINISTRATIVO|\s+DE\s+.{1,80}?|\s+REGISTRADO\s+E\s+PUBLICADO\s+SOB\s+O)?\b\s*N(?:\s*[°ºO])?\.?\s*(\d[\dA-Z.\-/]*)',

    r'\bEXTRATO\s+DE\s+CONTRATO\s*N[°ºO\.]?\s*([\d\.\-\/]+)',

    r'\bTERMO\s+DE\s+.{3,120}?\s+N[°ºO\.]?\s*([\d\.\-\/]+)',

    r'\bTERMO\s+DE\s+CESS[ÃA]O\s+DE\s+USO(?:\s+DE\s+IM[ÓO]VEL)?\s+(\d[\dA-Z\.\-\/]*)',
    ]
